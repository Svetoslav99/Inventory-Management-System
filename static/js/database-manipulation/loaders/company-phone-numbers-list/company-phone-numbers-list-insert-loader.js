...
const requestData = () => new Promise(async (resolve, reject) => {
    ...
    const phoneTypeValue = phoneType[0].value;
    const phoneNumberValue = phoneNumber[0].value;

    if (firstNameValue === "" || lastNameValue === "" || phoneTypeValue === "" || phoneNumberValue === "") {
        console.log('All fields are required!');
        return resolve({ success: false, message: 'All fields are required!' });
    } else {
        try {
            const response = await fetch(`/api/database-manipulation-homepage/phone-numbers-insert?
